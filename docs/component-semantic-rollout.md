...
register below, not in those files. `docs/semantic-token-mapping.md` is a
developer-facing education companion to this doc: it teaches the
role/intent-based reasoning model so new components can be classified without
waiting on a taxonomy update, and is the source of the G6/G7 gap entries below.
...
| Control                 | Input, Select, Checkbox, Radio, LabelStandalone, LabelWrapper, FormGroup, Accordion, Tabs (G6), Switch, SurfaceScroll                                    |
| Selection               | Chip, ToggleButton (including ToggleGroup) (G7), SurfaceSelection                                                                                        |
...
| G6  | Tabs interaction: shipped as `control` (switching tabs manipulates content in place, no navigation/submission); `docs/semantic-token-mapping.md` argues `selection` (choosing which content to view is a named-alternative choice, same intent as ToggleGroup) | Tabs                           | No render change either way; a future palette pass would apply the wrong role's tint/border weight if the wrong family ships                    | Design: confirm intent; eng: update contract if changed     |
| G7  | ToggleButton/ToggleGroup interaction: shipped as one `selection` contract for both; `docs/semantic-token-mapping.md` proposes splitting a standalone ToggleButton (no group, binary on/off) to `control`, keeping only ToggleGroup as `selection`              | ToggleButton, ToggleGroup      | No render change if standalone ToggleButton isn't actually used outside a group today; confirm real usage before treating this as required work | Design/eng: confirm standalone usage exists before deciding |
