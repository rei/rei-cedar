...
// Validate unwrap prop reactively so it catches prop changes too
watch(
  () => props.unwrap,
  (val) => {
    if (typeof val === 'string' && !propValidator(val, ['@xs', '@sm', '@md', '@lg'], false)) {
      console.warn(
        'CdrAccordionGroup: unwrap prop must be one of: @xs, @sm, @md, @lg, or a boolean',
      );
    }
  },
  { immediate: true },
...
const currentIdx = shallowRef<number>(0);
...
const unwrapped = shallowRef<boolean>(!!props.unwrap);
